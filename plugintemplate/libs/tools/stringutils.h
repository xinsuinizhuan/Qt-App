...
TOOLS_EXPORT QString settingsKey(const QString &category);

// Return the common prefix part of a string list:
// "C:\foo\bar1" "C:\foo\bar2"  -> "C:\foo\bar"
TOOLS_EXPORT QString commonPrefix(const QStringList &strings);

// Return the common path of a list of files:
// "C:\foo\bar1" "C:\foo\bar2"  -> "C:\foo"
TOOLS_EXPORT QString commonPath(const QStringList &files);
...
TOOLS_EXPORT QString withTildeHomePath(const QString &path);

// Removes first unescaped ampersand in text
TOOLS_EXPORT QString stripAccelerator(const QString &text);
// Quotes all ampersands
TOOLS_EXPORT QString quoteAmpersands(const QString &text);

TOOLS_EXPORT bool readMultiLineString(const QJsonValue &value, QString *out);

// Compare case insensitive and use case sensitive comparison in case of that being equal.
TOOLS_EXPORT int caseFriendlyCompare(const QString &a, const QString &b);

class TOOLS_EXPORT AbstractMacroExpander
{
public:
...
TOOLS_EXPORT void expandMacros(QString *str, AbstractMacroExpander *mx);
TOOLS_EXPORT QString expandMacros(const QString &str, AbstractMacroExpander *mx);

TOOLS_EXPORT int parseUsedPortFromNetstatOutput(const QByteArray &line);
