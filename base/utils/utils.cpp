...
void Utils::setQSS(const QString &qssNode)
{
    const QJsonObject json(jsonFromFile(qApp->applicationDirPath() + "/resource/config.json"));
    const QStringList qssPath(json.value(qssNode).toVariant().toStringList());
    QString qss;
    for (const QString &path : qAsConst(qssPath)) {
        qInfo() << QObject::tr("Loading QSS file: %1.").arg(path);
        QFile file(path);
        if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
            qWarning() << QObject::tr("Cannot open the file: %1!").arg(path) << file.errorString();
            continue;
        }
...
{
    const QDir dir(qApp->applicationDirPath() + "/resource/fonts/");
    if (!dir.exists()) {
        return;
    }
    QFontDatabase::removeAllApplicationFonts();
    const QFileInfoList fonts = dir.entryInfoList(QStringList("*.ttf"), QDir::Files);
    for (const QFileInfo &fileInfo : qAsConst(fonts)) {
        int fontId = QFontDatabase::addApplicationFont(fileInfo.absoluteFilePath());
        if (fontId == -1) {
            qWarning() << QObject::tr("Loading Fonts file: %1 Failed.").arg(fileInfo.fileName());
        } else {
            qDebug() << QObject::tr("Loading Fonts file: %1.").arg(fileInfo.fileName())
...
int Utils::getProcessIdFromName(const QString &name)
{
    int id = 0;
#ifdef Q_OS_WIN
    PROCESSENTRY32 pe;
