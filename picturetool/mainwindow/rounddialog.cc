    ...
    , d_ptr(new RoundDialogPrivate(this))
{
    ...
    setTitle(tr("Round Edit"));
    setRestoreMaxButtonVisible(true);
    setupUI();
    buildConnect();
    resize(parent->size());
    show();
...
{
    d_ptr->name = name;
    const QString title = tr("Round Edit-%1").arg(name);
...
void RoundDialog::setupUI()
{
    ...
    splitter->setSizes(QList<int>() << INT_MAX << 1);
...
QWidget *RoundDialog::toolWidget()
{
    ...
    startRoundButton->setFlat(true);
    startRoundButton->setObjectName("BlueButton");
    ...
    saveButton->setObjectName("BlueButton");
    connect(saveButton, &QPushButton::clicked, this, &RoundDialog::onSave);

    QWidget *toolWidget = new QWidget(this);
    toolWidget->setObjectName("InfoWidget");
